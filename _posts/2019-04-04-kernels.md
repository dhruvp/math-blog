...
The Kernel of a function is the set of points that the function sends to $0$. Amazingly, once we know this set, we can immediately characterize how the linear function maps its inputs to its outputs. It's an extremely powerful idea that will help us see matrices and linear functions in a new light.
