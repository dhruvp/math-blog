...
Let’s now simplify this formula a little. Let’s call the set of all friends $F$. And let $IsFriends(p, Fred)$ be $1$ if $p$ and $Fred$ are friends and $0$ otherwise. Then, the above formula is equivalent to:
