...
Linear functions are the foundations of matrices. One example is $f(x) = 3x$ for real numbers $x$. Their main rules are as follows:
* $ f(c \cdot x) = c \cdot f(x) $
   * Can multiply by a scalar before or after applying by the function and get the same result.
   * For our example, $f(5\cdot x) = 3 \cdot 5x = 15x = 5 \cdot f(x)$.
* $ f(x+y) = f(x) + f(y) $
   * Can add before or after applying the function and get the same result.
   * For our example, $f(x + y) = 3 \cdot (x + y) = 3x + 3y = f(x) + f(y)$.
...
* Rotations
   * A function that rotates an shape around a point.
...
* Scaling
   * A function that takes a 2d image and makes it 2x as large.
...
So we now know what linear functions are and we've seen how they represent many physical phenomena - but how can we succintly represent them? Taking a step back, how do we represent functions in general? One way is to simply have a table that takes every input and displays its output. For instance, we could write:
...
Let's call the first column of $F$ $F_{col1}$ and the second column $F_{col2}$.
...
Well we just saw earlier that this is just $G \cdot F_{col1}$. So we can now write:

$$H = \begin{bmatrix} G \cdot F_{col1} & G \cdot F_{col2} \end{bmatrix} $$

Seem familiar? Because it’s exactly what the formula for the 2x2 matrix multiplication $H = G\cdot F$! So $H$, the matrix representing $g(f(x))$, is just $G\cdot F$. **This shows that matrix multiplication can be thought of as function composition.**

Thanks to matrix multiplication, we have amazingly simple rules for understanding $g \circ f$, $f+g$ etc. without manually writing out a bunch of tables.
